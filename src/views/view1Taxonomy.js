...
export function initView1(container, { manifest }) {
  ...
  const render = (availableWidth) => {
    ...
    const animated = narrow ? { x: 26, y: 62, width: 588, height: 310 } : { x: 26, y: 62, width: 790, height: 220 };
    appendSvg(svg, "rect", { ...animated, rx: 14, class: "taxonomy-container" });
    labelBlock(svg, animated.x + 24, animated.y + 45, corpus.animated_films, "Primary animated comparison", "Disney animated and DreamWorks", colors.primaryText);
    const disney = narrow ? { x: 50, y: 150, width: 540, height: 112 } : { x: 50, y: 150, width: 475, height: 108 };
    appendSvg(svg, "rect", { ...disney, rx: 10, class: "taxonomy-subcontainer taxonomy-subcontainer--disney" });
    const studioCounts = corpus.studio_counts;
    labelBlock(svg, disney.x + 18, disney.y + 34, corpus.disney_animated_films, labels.disneyAnimatedAggregate, `${studioCounts["Walt Disney Animation Studios"]} WDAS + ${studioCounts["Pixar Animation Studios"]} Pixar`, colors.disneyAnimatedAggregate);
    drawFilmMark(svg, filmShapes["Walt Disney Animation Studios"], disney.x + 26, disney.y + 88, 6, { fill: colors.wdas });
    appendSvg(svg, "text", { x: disney.x + 40, y: disney.y + 93, class: "chart-note" }, `${studioCounts["Walt Disney Animation Studios"]} Walt Disney Animation Studios`);
    drawFilmMark(svg, filmShapes["Pixar Animation Studios"], disney.x + (narrow ? 302 : 270), disney.y + 88, 6, { fill: colors.pixar });
    appendSvg(svg, "text", { x: disney.x + (narrow ? 316 : 284), y: disney.y + 93, class: "chart-note" }, `${studioCounts["Pixar Animation Studios"]} Pixar Animation Studios`);
    const dwa = narrow ? { x: 50, y: 280, width: 540, height: 68 } : { x: 548, y: 150, width: 244, height: 108 };
    appendSvg(svg, "rect", { ...dwa, rx: 10, class: "taxonomy-subcontainer taxonomy-subcontainer--dwa" });
    drawFilmMark(svg, filmShapes["DreamWorks Animation"], dwa.x + 22, dwa.y + 27, 6, { fill: colors.dreamworks });
    appendSvg(svg, "text", { x: dwa.x + 38, y: dwa.y + 32, class: "taxonomy-label" }, `${corpus.dreamworks_animated_films} DreamWorks Animation`);
    if (!narrow) appendSvg(svg, "text", { x: dwa.x + 22, y: dwa.y + 65, class: "chart-note" }, "Animated comparison side");
    const remake = narrow ? { x: 26, y: 394, width: 588, height: 90 } : { x: 840, y: 62, width: 214, height: 220 };
